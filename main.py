                ...
                for pattern in sorted(set(match[0] for match in re.findall('(\d+-\d+(-\d+)?(\.\d+(-\d+)?(\.\d+)?)?)', tag.text.strip()))):
                    if re.search('\d+-\d+-\d+\.\d+',pattern):
                        ...
                        if self.soup.find("h3",id=id):
                            ...
                            tag_string = re.sub(pattern,f"<cite><a href=#http://localhost:63342/practice/venv/ricode/modified/gov.ri.code.title.{section_match.group('title_id').zfill(2)}.html?_ijt=lartillgujbilc2c7ak6tlmhr8&_ij_reload=RELOAD_ON_SAVE#{tag_id}>{pattern}</a></cite>", text)
                        tag.clear()
                        tag.string = tag_string
                    else:
                        section_match = re.search("(?P<title_id>\d+)-(?P<chapter_id>\d+)(-\d+)?(\.\d+(-\d+)?(\.\d+)?)?",pattern)
